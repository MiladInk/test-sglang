...
if __name__ == '__main__':
    ...
    torch.cuda.set_device(rank) # important for all_gather_object as it relies on user provided device
    
    dataset = load_dataset("MathMindsAGI/MATH-openai-split")
    ds = dataset["train"]
    ds = ds.select(range(10)) # for debugging purposes
    if rank == 3:
        import debugpy
        debugpy.listen(5678)
        print("Waiting for debugger attach")
        
    llm = sgl.Engine(model_path="realtreetune/deepseekmath-7b-sft-MATH-v2", device="cuda", dp_size=1, base_gpu_id=rank)
    print(f"Model loaded-{rank}")
    start_time = time.time()
    
    ds = ds.shard(num_shards=world_size, index=rank)
    ds = generate_responses(ds)
    ...
    # cast prefixes_mcs_output to json string, or else it will fail the concatenation as the structure is not necessarily the same
    mc_ds = mc_ds.map(lambda x: {'prefixes_mcs_output': json.dumps(x['prefixes_mcs_output'])})

    if rank == 0:
        mc_ds_list = [None] * world_size
        dist.gather_object(mc_ds, mc_ds_list, dst=0)
        # print(f"rank: {rank}, received all the shards, len(mc_ds_list): {len(mc_ds_list)}")
        mc_ds = datasets.concatenate_datasets(mc_ds_list)
        mc_ds.save_to_disk("./vine_episodes")
    else:
        dist.gather_object(mc_ds, None, dst=0)
        # print(f"rank: {rank}, sent the shard")
